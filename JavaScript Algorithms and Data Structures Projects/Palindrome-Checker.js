...
function palindrome(str) {
    // Good luck!
    var myArr = str.split("");
    var alphaArr = myArr.filter(function(item, index) {
        return item.match(/[a-z0-9]/i);
    });

    var toLower = alphaArr.map(function(item, index) {
    ...
    do {
        truthArr.push(toLower[i] === toLower[(toLower.length - 1) - i]);
        i++
    } while (i < Math.floor(toLower.length / 2));

  

    var bool = truthArr.every(function(item, index) {
        return item === true;
    });
